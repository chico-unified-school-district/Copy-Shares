...
ForEach ( $job in $jobs ) {
	...
	if ( !(Test-Path $srcSharePath) -or !(Test-Path $dstSharePath ) ) {
		...
		Report-Msg -msg "$srcSharePath $dstPath $options /XD $excludeDirs /XF $excludeFiles /LOG:$RoboLogFile /W:0 /R:0 /NFL /NDL /XA:S $testSwtich"
		# ROBOCOPY $srcSharePath $dstPath $options /XD $excludeDirs /XF $excludeFiles /LOG:$RoboLogFile /W:0 /R:0 /NFL /NDL /XA:S $testSwtich
		".\bin\FastCopy211_x64\FastCopy.exe"
	}
