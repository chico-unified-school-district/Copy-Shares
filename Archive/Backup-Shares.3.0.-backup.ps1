...
ForEach ( $job in $jobs ) {
	...
	if ( !(Test-Path $srcSharePath) -or !(Test-Path $dstSharePath ) ) {
		...
		$dstPath = "\\$dstServer\$dstShare\$scope\$srcServer\$srcShare"
   ...
   Get-ChildItem -Path .\logs\$srcServer -Recurse -Force | Where-Object { !$_.PSIsContainer -and $_.CreationTime -lt $limit } | Remove-Item -Force -Whatif:$test
		...
		Report-Msg -msg "ROBOCOPY $srcSharePath $dstPath $options /XD $excludeDirs /XF $excludeFiles /LOG:$RoboLogFile /W:0 /R:0 /NFL /NDL /XA:S $testSwitch"
